...
📤 ОТВЕТЬ ТОЛЬКО JSON-МАССИВОМ С ПОЛНЫМ ТЕКСТОМ ВАРИАНТОВ:`;
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            const hasInvalidPattern = aiVariants.some(v => invalidPatterns.some(pattern => pattern.test(v)));
            ...
            for (const aiVariant of aiVariants) {
                ...
                const foundOriginal = options.find(opt => opt === aiVariant || window.normalizeText(opt) === normalizedAi);
                if (foundOriginal) {
